...
function SinglePopularSandwich({sandwich}) {
    ...
    return (
        <div onClick={()=>router.push(`kanapka_${sandwich?.id}`)} className="justify-end items-end rounded-xl mr-5 w-80 h-52 border-2 flex gap-52 bg-cover cursor-pointer flex-shrink-0 flex-grow-0" style={{backgroundImage:`url("${sandwich?.img}")`}}>
